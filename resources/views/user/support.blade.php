        ...
        <div class="col-md-6 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    ...
                    <p class="card-description"> @lang('message.body.contact_us') </p>
                    <form class="forms-sample" method="post" action="{{ route('enquiry') }}">
                        <input type="hidden" name="name" value="{{ Auth::user()->name }}" />
                        <div class="form-group">
                            <input type="hidden" name="email" value="{{ Auth::user()->email }}">
                        </div>
                        <div class="form-group">
                            <label for="">@lang('message.body.subj')</label>
                            <input type="text" class="form-control" id="" placeholder="Subject" name="subject">
                        </div>
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        <div class="form-group">
                            <label for="">@lang('message.body.mes')</label>
                            <textarea class="form-control" id="" rows="4" name="message"></textarea>
                        </div>
                        <a type="submit" class="btn btn-gradient-primary me-2">@lang('message.body.send')</a>
                        <input type="hidden" name="id" value="{{ Auth::user()->id }}">
                                    <input type="hidden" name="current_password" value="{{ Auth::user()->password }}">
                                    <input type="hidden" name="_token" value="{{ csrf_token() }}"><br />
                    </form>
